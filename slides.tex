...
\begin{frame}{Pensemos un ejemplo}
	...
	\begin{center}
		\begin{tikzpicture}[scale=0.75]
			...
			\only<8>{
				\node[diamond state, fill=gray!30] (current2) at (5.5, 1) {$(0,0)$ \\ $(1,0)$};
			}
		\end{tikzpicture}
	\end{center}
	\only<9>{
...
\begin{frame}{Politopos}
	\begin{block}{Politopo}
		Un politopo en $\mathbb{R}^n$ es un conjunto acotado $K = \{ x
	...
	\begin{itemize}
		\item Las funciones en $\mathbb{R}^S$ se pueden pensar como vectores y podemos hablar
		      ...
		      %       convexo. \pause
		\item  Definimos como \( \DPoly(S) \) al conjunto de politopos cuyos elementos son
		      funciones de probabilidad sobre $S$.
	\end{itemize}
...
\begin{frame}{Ejemplo de juego estoc\'astico polit\'opico}
	...
	\begin{tikzpicture}[->, scale=0.75]
		...
		\node[align=left, anchor=west] at (8, -3) {
			...
			\textcolor{violet}{$0 < p_3 \leq 0.2$}\\
			\textcolor{violet}{$p_1 > 0$} \\
			\textcolor{orange}{$p_1 + p_2 + p_3 = 1$}\\
			\textcolor{orange}{$0.05 \leq p_2 \leq 0.3$}\\
			\textcolor{orange}{$0 < p_3 \leq 0.2$}\\
			\textcolor{orange}{$p_1 > 0$}
		};
		\pause
...
\begin{frame}{Estrategias}
	...
	\only<1,2>{
		\begin{block}{Estrategia en un PSG}
			Una estrategia $\pi_i$ para el jugador $i$ ($i \in \{\cuad, \diam\}$) en un PSG ser\'a una funci\'on $\pi_{{i}} : \Pathsfin^{i} \rightarrow {\PMeas}({\Act})$ que asigna una {medida de probabilidad} sobre las {acciones} a cada ${\omega} {s}$.
		\end{block}}
...
\begin{frame}{Objetivos en un juego}
	...
	\begin{itemize}
		\item maneras de codificar la especificaci\'on deseada para nuestro sistema/modelo
		      \pause
		\item conjuntos de comportamientos/caminos (secuencias de estados) \pause
	\end{itemize}
	...
	\pause
	Los podemos expresar en LTL (recordando que a $\diam$ lo podemos leer como ``eventualmente'' y a $\cuad$ como ``siempre").
...
\begin{frame}{La idea general}
	...
	\begin{enumerate}
		...
		\item Probamos que los conjuntos ganadores para $\cuad$ en ambos juegos son
		      equivalentes. \pause
	\end{enumerate}
	Con esto, adaptamos un \textbf{algoritmo} de ejecuci\'on simb\'olica para el c\'alculo de este conjunto, analizamos su \textbf{complejidad} y damos un m\'etodo para la obtenci\'on de \textbf{estrategias ganadoras}.
\end{frame}
...
\begin{frame}{Procesos de Decisi\'on de Markov Polit\'opicos}
	\begin{block}{Proceso de Decisión de Markov Politópico}
		Un Proceso de Decisión de Markov Politópico (PDMP) es un PSG donde consideramos un solo jugador, $\St_\diam = \emptyset$ o
		$\St_\cuad = \emptyset$.
	\end{block}

	\pause
	\begin{block}{Sub-PMDP}
		Un sub-PMDP es un par $(C, D)$, donde $C \subseteq S$ y $D: C \rightarrow \mathcal{P}(\mathcal{P}(S))$ es una función que asocia a cada $s \in C$ un conjunto $D(s) \subseteq \Vs$ de subconjuntos de estados próximos posibles.
	\end{block}
...
\begin{frame}{Procesos de Decisi\'on de Markov Polit\'opicos}
	...
	\begin{block}{Corolario}
		Una condición de Rabin $R = \{ (E_1, F_1), \dots, (E_d, F_d)\}$ se satisface con probabilidad 1 desde $s$ siguiendo una estrategia de memoria finita $\pi$ si y solo si para cada componente final $(C,D)$ alcanzable desde $s$, existe un $j \in \{ 1, \dots, d\}$ tal que $C \cap E_j = \emptyset$ y $C \cap F_j \neq \emptyset$.
	\end{block}
\end{frame}
...
\begin{frame}{Sint\'esis de estrategia ganadora en un PSG}
	...
	{\small
		...
		\begin{itemize}
			\pause
			\item El algoritmo simbólico que presentamos permite extraer una \textbf{estrategia
				      sin memoria} $\sigma^{*}_{\Box}$ ganadora para $\Box$ en $\DGk$. Esta
			      estrategia indica, para cada estado $s \in \St_\cuad$, qué vértice $v_{V'}$
			      debe elegirse.
		\end{itemize}
