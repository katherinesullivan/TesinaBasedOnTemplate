...
Llamaremos proceso de decisión de Markov politópico a un PSG donde $\St_\cuad =
	\emptyset$ o $\St_\diam = \emptyset$. Al igual que en el caso de PSGs, haremos
el estudio de PMDP sobre la interpretación de los mismos que formalizamos de la
siguiente manera:

\begin{definition}
	La interpretación de un \textit{proceso de decisión de Markov politópico} (PMDP por sus siglas en inglés) es una tupla $\M = (\St, Act, \theta')$ donde $S$ es un conjunto finito de estados, $Act$ es un conjunto de pares (politopo, distribución) y \\ $\theta': \St \times Act \times \St \rightarrow [0, 1]$ es la función de transición entre estados definida como en la Definición~\ref{def:interpret}.
\end{definition}
...
\begin{definition}[Conjuntos estado-resultado y sub-PMDPs]
	Sea $\M=(S, Act, \theta')$ un PMDP y dado un estado $s$, sea $\Vs = \{\supp(\mu) | (K, \mu) \in \Act(s)\}$ el conjunto formado por los conjuntos soporte de las acciones salientes de él. Un conjunto estado-resultado es un subconjunto $\chi \subseteq \{(s, V') \mid s \in S \wedge V' \in \Vs\}$, mientras que un sub-PMDP es un par $(C, D)$, donde $C \subseteq S$ y $D: C \rightarrow \mathscr{P}(\mathscr{P}(S))$ es una función que asocia a cada $s \in C$ un conjunto $D(s) \subseteq \Vs$ de subconjuntos de estados próximos posibles. Hay una relación uno-a-uno entre sub-PMDPs y conjuntos de estado-acción:
...
\begin{theorem}[Estabilidad de componentes finales]
	...
	\begin{equation} \label{estabEC}
		\Prob^\pi_s(\diam C) = \Prob^{\pi'}_s(\diam C) = \Prob^{\pi'}_s(\{ \omega \in \Paths_s | \inft(\omega) = \er(C,D)\})
	\end{equation}
	para todo $s \in S$.
\end{theorem}
...
\begin{proof}
	...
	\begin{itemize}
		\item Si $s_n \in C$, la estrategia asignará probabilidad positiva a una única acción
		      $(K, \mu) \in \Act(s_n)$ tal que $\supp(\mu)=V'$ para cada conjunto resultado
		      $V' \in D(s_n)$ (denotaremos a esta acción particular con $(K, \mu)_{V'}$), y
		      la probabilidad de elegir cada una de esas acciones se distribuirá de manera
		      uniforme. Es decir,
		      \[
...
\begin{theorem}[teorema fundamental de las componentes finales] \label{teoFundEC}
	...
	$$\Prob^\pi_{\M, s}(\{ \omega \in \Paths_s \mid \sub(\inft(\omega)) \text{ es una componente final}\}) = 1$$
\end{theorem}
...
\begin{proof}
	...
	\begin{equation}\label{omegaEnOmega0}
		\Prob^\pi_{\M, s}(\{ \omega \in \Paths_s \mid \omega \in \Paths_s^{(C,D)}\}) = 0,
	\end{equation}
	...
	\begin{itemize}
		\item Primero, asumamos que existe un $(t, V') \in \er(C,D)$ tal que $V' \nsubseteq
		      ...
		      que $r < 1$, tenemos que $$\Prob^\pi_{\M, s}(\{ \omega \in \Paths_s \mid \omega
			      \in \Paths_s^{(C,D)}\}) = 0.$$
...
\subsection*{Nota sobre la limitación a estrategias finitas del Teorema~\ref{teoFundEC}}
...
\begin{align*}
	&\#_t(\varepsilon) = 0 \\
	&\#_t(\omega \alpha t) = \#_t(\omega) + 1 \\
	&\#_t(\omega \alpha s) = \#_t(\omega)
\end{align*}
siendo $\omega$ un prefijo de camino, $\alpha$ una acción, $\varepsilon$ el
camino vacío y $s$ y $t$ estados diferentes.
...
% ganadoras que mostramos más adelante, porque terminamos arribando a otra forma
% de probarlo.
% pudimos reducir lo que
