...
\begin{boxgris}[\emph{Roborto}, el robot probabilístico]{}
	Supongamos que queremos observar el comportamiento de un pequeño robot llamado \textit{Roborto}.

	\textit{Roborto} se encuentra en una grilla 2x2 y se comporta totalmente probabilísticamente siguiendo la siguiente descripción: desde su posición inicial de quietud en la esquina superior izquierda tiene una probabilidad de 0.5 de seguir allí y 0.25 de moverse tanto a la izquierda como hacia abajo. Si sucede que en algún momento se encuentra en la esquina superior o inferior derecha, como a \emph{Roborto} no le gusta quedarse del lado de la derecha, tiene probabilidad 0 de quedarse allí o de ir hacia arriba o abajo, respectivamente. Desde esas esquinas, Roborto se mueve con probabilidad 1 hacia la izquierda. Desde la esquina inferior izquierda tendrá una probabilidad de 0.3 de quedarse allí, 0.2 de ir a la derecha y 0.5 de ir hacia arriba.

	El comportamiento de Roborto se puede modelar con la cadena de Markov que
	...
	\begin{figure}[h]
		...
		\end{tikzpicture}
		\caption{Cadena de Markov representando el comportamiento probabilístico de \emph{Roborto}.}
		\label{fig:cm-roborto}
	\end{figure}

	Caminos posibles de esta cadena de Markov podrían ser el que mantiene a
	\emph{Roborto} siempre en la esquina superior izquierda, $(SI, SI, SI, \dots)$,
	el describe un movimiento en ``C" de \emph{Roborto}, $(SI, SD, SI, II, ID, II,
		SI, SD, SI, II, ID, II, \dots)$ o algunos de los que mantienen a \emph{Roborto}
	en la parte superior de la grilla, como $(SI, SD, SI, SD, SI, SD, \dots)$ o
	$(SI, SD, SI, SD, SI, SI, SI, \dots)$.

	Una duda natural entonces que podría surgir es ¿qué probabilidad hay de que
	\emph{Roborto} siga alguno de esos caminos?
\end{boxgris}

Para poder asociar probabilidades a eventos en cadenas de Markov (o, tal vez
pensar en preguntas sobre la probabilidad de que \emph{Roborto} siga algún tipo
particular de camino), la noción intuitiva de probabilidades en $M$ es
formalizada al asociarle un espacio de probabilidad. Los caminos infinitos de
$M$ juegan el rol de resultados de la $\sigma$-álgebra asociada. Esto es
$Outc^M = \paths(M)$. La $\sigma$-álgebra asociada con $M$ es generada pot los
conjuntos cilindro formados por los fragmentos de caminos finitos en $M$.
