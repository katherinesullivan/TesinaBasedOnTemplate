...
\begin{proof}
	...
	1, pues veamos la siguiente derivación:
	...
	Para probar que esta sumatoria da 0, veremos que cada término en esta es $0$.
	Para eso, consideremos un $V' \in \VGk$ arbitrario y también un $s' \in V'$
	arbitrario.

	Sea $I = (i_0, i_1, i_2, \dots)$ el conjunto infinito de momentos donde se
	elige alguna acción $(K,\mu)$ cuyo soporte es $V'$. \kathy{¿Decir algo más de
		la justificación de pq decimos que existe este conjunto infinito I?} Como
	estamos trabajando con estrategias de memoria finita habrá solo una cantidad
	finita de distribuciones que se pueden elegir desde $s$. Por lo tanto, existirá
	una distribución $\mu_\text{min}$ que minimiza la probabilidad de ir a $s'$.
	...
	$l$ momentos en $I$, es decir, en ninguno de los momentos $(i_{k+1}+1,
		...
		\mu_\text{min})^l$, que converge a 0 cuando $l$ tiende a infinito. Esto
	...
	$\sigma_\diam$ arbitraria válida en $\DGk$, $\Inf_C(\rho) = \Inf(\rho) \cap
		\St$ cumple con la especificación $\varphi$. Es decir, que vale:
	...
	\begin{itemize}
		\item Para cada $V'$ que se asigna con $\Inf_D(\rho)$ vale que $V' \subseteq
		      ...
		      una estrategia valida en $\DGk$, pues se visitaría infinitas veces $v_{V'}$
		      pero solo finitas veces $s'$, uno de sus sucesores.
		\item El grafo $(\Inf_C(\rho) \cup \{v_{V'} | \exists s \in C : V' \in
			      \Inf_D(\rho)(s)\}, \rho_{(\Inf_C(\rho), \Inf_D(\rho))})$ es trivialmente
		      conexo, pues cualesquiera dos vértices $u$, $v$ pueden ser conectados con el
		      camino $\rho$ al ambos aparecer infintas veces en él.
	\end{itemize}
