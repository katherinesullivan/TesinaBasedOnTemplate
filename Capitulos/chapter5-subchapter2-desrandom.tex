...
vértices ganadores con probabilidad 1 en un juego estocástico politópico
\kathy{agg si nos reducimos a analizar solo estrategias de memoria finita?} con
un objetivo de Rabin es igual al conjunto de vértices ganadores de un juego
justo construido a partir del PSG.
...
Sea $G = (V, (V_\cuad, V_\diam), E)$ un juego de grafo de dos jugadores y sea
...
salientes hacia los estados $q$ y $q'$, como se muestra en la
Figura~\ref{fig:juego-justo} (parte inferior). Si el jugador $\diam$ elige
aleatoriamente entre las aristas $(p, q)$ y $(p, q')$, toda secuencia finita de
visitas a los estados $q$ y $q'$ ocurrirá infinitamente a menudo con
probabilidad uno. Esto no es cierto en el juego de adversario justo. Aquí el
jugador $\diam$ puede elegir una secuencia particular de visitas a $q$ y $q'$
(por ejemplo, simplemente $qq'qq'qq'\dots$), siempre que ambos sean visitados
...
Dada la interpretación de un juego estocástico politópico $\Gk = (\St,
	(\St_\cuad, \St_\diam), \Act, \theta)$, y denotando con $\Vs$ al conjunto
$\{\supp(\alpha) | \alpha \in \Act(s)\}$ para cada $s \in \St$, se define la
\textit{desrandomización} de $\Gk$, $\DGk := (( V, V_\cuad, V_\diam, E ), E^l
	)$, como el juego de grafo de 2 jugadores justo con:
...
juego justo en el juego estocástico politópico fácilmente usando notación LTL
sorbe caminos abstractos.
...
\begin{proof}
	...
	finito $\hat \rho = (s_0, v_{V_0}, s_1, v_{V_1}, \dots, s_n)$ con $s_n \in
		\St_\cuad$, asigna como próximo vértice un a un vértice $v_V'$ que se
	...
	\begin{itemize}
		...
		\item El grafo $(\Inf_C(\rho) \cup \{v_{V'} | \exists s \in C : V' \in
		      ...
		      camino $\rho$ dado que ambos aparecer infintas veces en él.
	\end{itemize}
...
sabemos que el conjunto $\nu Y .\ \mu X.Q \cap [(F \cap \Cpre(Y) \cup
		(\Cpre(X)))]$ define exactamente a los estados ganadores para el jugados
$\cuad$ en un juego de grafo de dos jugadores tradicional con objetivo $\psi$
\cite{MalerSynthesis}. El operador $\Apre$ lo que hace es adaptar este cómputo
para los juegos con adversario justo. Con lo que se puede entender que cada
término $C_{p_j}$ resuelve el problema local de quedarse en $Q_{p_j}$ y volver
siempre eventualmente a $F_{p_j}$, y los sucesivos puntos fijos máximos y
mínimos permiten reflejar la semántica de los objetivos de Rabin.
