...
\begin{proof}
	...
	a cada prefijo de camino en $\Gk$ $\hat \rho^n$ de longitud $n$ le asigna $\hat
		\omega^{n+1}$, el prefijo de camino finito de lomngitud $n+1$ del menor camino
	ganador $\omega$ tal que $\derand(\omega_n) = \hat \rho^n$, tomando como orden
	uno que asigna mayor precedencia a todos los caminos ganadores y luego se
	...
	Si tenemos que $s_\rho \in \Inf(\rho)$ entonces sabemos que $\exists^{\infty} n
		\mid \left( \sigma_\cuad^*(\rho_n) = v_{V'} \vee \sigma_\diam(\rho_n) =
		v_{V'}\right)$ para algún $V'$ tal que $s_\rho \in V'$.

	Si $\exists^{\infty} n \mid \sigma_\cuad^*(\rho_n) = v_{V'}$ para algún $V'$
	tal que $s_\rho \in V'$, entonces por como definimos $\sigma_\cuad^*$ eso
	significa que $\exists^{\infty} n \mid \pi_\cuad^*(\omega_n) = \alpha'$ para
	algún $\alpha'$ tal que $\supp(\alpha') = V'$ y $s_\rho \in V'$, con lo que
	$s_\rho$ pertenecería también a $\Inf(\omega)$ por la condición de equidad
	impuesta sobre los juegos justos.

	Si esto no sucede, entonces necesariamente ocurre que $\exists^{\infty} n \mid
		\sigma_\diam(\rho_n) = v_{V'}$ para algún $V'$ tal que $s_\rho \in V'$. Pero
	entonces, como $\pi_\diam$ es solo la randomización de $\sigma_\diam$ tenemos
	que $\exists^{\infty} n \mid \pi_\diam(\omega_n) = \alpha'$ para algún
	$\alpha'$ tal que $\supp(\alpha') = V'$ y $s_\rho \in V'$, con lo que $s_\rho$
	pertenecería también a $\Inf(\omega)$ por la condición de equidad impuesta
	sobre los juegos justos.

	Un análisis idéntico se puede realizar para ver que dado un $s_\omega \in
		\Inf(\omega) \cap \St$ este pertenece también a $\Inf(\rho)$, con lo que se
	prueba la doble contención que muestra la igualdad entre $\Inf(\rho) \cap \St$
	y $\Inf(\omega) \cap \St$, y con eso lo que queríamos probar originalmente.
