...
$s_i\in\psgnodes$ y $V_i\in \{\supp(\alpha) | \alpha \in \Act(s_i)\}$, $0\leq i
	< n$, que capturan los caminos donde las respectivas acciones tienen medidas de
probabilidad cuyo soporte son los conjuntos $V_i$. Esto queda formalizado como
sigue.

Dado $V\in \{\supp(\alpha) | \alpha \in \Act(s)\}$, definimos
...
Podemos expresar la condición de equidad representativa del juego justp en el
juego estocástico politópico fácilmente con notación LTL sorbe caminos
abstractos. Si denotamos $V_{\Gk}$ al conjunto finito de todos los conjuntos
soporte de medidas de probabilidad de las acciones en $\Gk$, tenemos que:

\begin{center}
	$
		\hat \varphi^l := \bigwedge_{V' \in V_{\Gk}} \hat \varphi^{V'}
	$, con
...
\kathy{¿Algo más para que quede obvio que es igual a varphi l?}
...
\begin{proof}
	...
	$\hat \varphi^l = \bigwedge_{V' \in V_{\Gk}} \bigwedge_{s' \in V'} \left( \siempevent V' \rightarrow \siempevent (V' \wedge \bigcirc s') \right).$
	...
	\begin{align*}
		&\Prob_{\Gk, s}^{\picuadGk, \pidiam}(\neg \hat{\varphi}^l) = \\
		&\Prob_{\Gk, s}^{\picuadGk, \pidiam}\left(\neg \left( \bigwedge_{V' \in V_{Gk}} \bigwedge_{s' \in V'} \left( \siempevent V' \rightarrow \siempevent (V' \wedge \bigcirc s' ) \right)\right)\right) = \\
		&\Prob_{\Gk, s}^{\picuadGk, \pidiam}\left(\left( \bigvee_{V' \in V_{Gk}} \left( \neg \bigwedge_{s' \in V'} \left( \siempevent V' \rightarrow \siempevent (V' \wedge \bigcirc s' ) \right)\right)\right)\right) = \\
		&\Prob_{\Gk, s}^{\picuadGk, \pidiam} \left(\bigvee_{V' \in V_{Gk}} \bigvee_{s' \in V'} \neg \left( \siempevent V' \rightarrow \siempevent (V' \wedge \bigcirc s' ) \right)\right) = \\
		&\Prob_{\Gk, s}^{\picuadGk, \pidiam} \left(\bigvee_{V' \in V_{Gk}} \bigvee_{s' \in V'} \left( \siempevent V' \wedge \eventsiemp \neg (V' \wedge \bigcirc s' ) \right)\right) \leq \\
		&\sum_{V' \in V_{\Gk}} \sum_{s' \in V'} \Prob_{\Gk, s}^{\picuadGk, \pidiam} \left( \siempevent V' \wedge \eventsiemp \neg (V' \wedge \bigcirc s' ) \right)
	\end{align*}

	Para probar que esta sumatoria da 0, veremos que cada término en esta es $0$.
	Para eso, consideremos un $V' \in V_{\Gk}$ arbitrario y también un $s' \in V'$
	arbitrario.
	...
	\begin{equation}
		\label{varphi'}
		\varphi' := \bigvee_{j = 1}^{d} \left((\infsigma \cap E_j = \emptyset) \wedge (\infsigma \cap F_j \neq \emptyset)  \right)
	\end{equation}
	...
	Ahora veamos que esta componente final es alcanzable en $\M^*$. Esto lo veremos
