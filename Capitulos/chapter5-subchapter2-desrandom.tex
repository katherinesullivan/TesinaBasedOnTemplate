...
\begin{proof}
	...
	$\sigma_\diam$ que respeta la condición de equidad $\varphi^l$, etnonces la
	...
	$\hat{\varphi^l} = \bigwedge_{V' \in V_{\Gk}} \bigwedge_{s' \in V'} \siempevent V' \rightarrow \siempevent (V' \wedge \bigcirc s').$

	Entonces, en principio, tenemos que:

	$\inf_{\pidiam \in \Pidiam} \Prob^{\picuadGk, \pidiam}_{\Gk, s} (\hat{\varphi^l} \rightarrow \varphi) = 1.$

	Veremos que la ecuación~\ref{pisirve} vale viendo que la probabilidad de que no
	valga $\hat{\varphi^l}$ es $0$.
	...
	Veamos que cada término en esta última sumatoria es $0$. Para eso, consideremos
	un $V' \in V_{\Gk}$ arbitrario y también un $s' \in V'$ arbitrario.
	...
	Con esto tenemos que $\Prob = 0$ y ... lo que queríamos probar
	...
	Definimos $\sigma_\cuad^* = \derand(\picuad^*, f)$, siendo $f$ una función que
	a cada prefijo de camino en $\DGk$ $\hat \rho^n$ de longitud $n$ le asigna
	$\hat \omega^{n+1}$, el prefijo de camino finito de longitud $n+1$ del menor
	camino ganador $\omega$ tal que $\derand(\omega_n) = \hat \rho^n$, tomando como
	orden uno que asigna mayor precedencia a todos los caminos ganadores y luego se
	comporta de acuerdo al orden de caminos presentado en la subsección anterior de
	esta sección \ref{ordenCaminos}.
