...
~\label{cap:psg}
\vspace{-1cm}
\epigraph{Infinito: mayor que la cosa más grande que haya existido nunca, y más. Mucho mayor que eso, en realidad; verdadera y asombrosamente enorme, de un tamaño absolutamente pasmoso, algo para decir: ``vaya, qué cosa tan inmensa".}{\textit{El restaurante del fin del mundo}, \\ Douglas Adams}

El propósito de este capítulo es presentar las definiciones y los hallazgos que
nos son más relevantes de la reciente investigación de Castro y D'Argenio
\cite{Polytopal}. Dicho trabajo es el primero en presentar el concepto de
...
desigualdades lineales cuyas soluciones forman un politopo.
...
$\mathbb{R}^{|S|}$, generalmente nos referiremos a politopos en $\mathbb{R}^S$.
...
en \( S \) forma el politopo convexo $$ \Dist(S) = \{ \mu \in \mathbb{R}^S \mid
	\sum_{s \in S} \mu(s) = 1 \text{ y } \forall s \in S : \mu(s) \geq 0 \}. $$
...
\subsection*{Juegos estocásticos politópicos}
...
\begin{definition}[Juego estocástico politópico]
...
Un PSG se diferencia de un juego estocástico tradicional por el hecho de que
desde un estado \( s \in \St_i \) (para \( i \in \{ \square, \diamondsuit \}
\)), el jugador \( i \) elige jugar un politopo \( K \in \Theta(s) \) y una
distribución \( \mu \in K \), en vez de elegir directamente una acción de entre
un conjunto finito. Al igual que en un juego estocástico, el siguiente estado
\( s' \) se selecciona de acuerdo con la distribución \( \mu \) correspondiente
a la acción seleccionada, y el juego continuará desde \( s' \) repitiendo el
mismo proceso.
...
\begin{definition}[Interpretación de un PSG]
	\label{def:interpret}
	La interpretación del PSG \( \K \) se define por el juego estocástico \( \Gk = (\St, (\St_\square, \St_\diamondsuit), \Act, \theta) \), donde \\ \(\Act = \bigcup_{s \in \St} \Theta(s) \times \textsf{Dist}(\St) \) y
	\[
		\theta(s, (K, \mu), s') =
		\begin{cases}
...
Para esto, daremos una construcción estándar para darle a \( \textsf{Dist}(\St)
\) una \( \sigma \)-álgebra: \( \Sigma_{\textsf{Dist}(\St)} \) se define como
la \( \sigma \)-álgebra más pequeña que contiene los conjuntos \\ \( \{\mu \in
\textsf{Dist}(\St) \mid \mu(S) \geq p\} \) para todo \( S \subseteq \St \) y \(
p \in [0, 1] \) (ver~\cite{Giry82}). Ahora, dotamos a \( \Act \) con la \(
\sigma \)-álgebra producto \( \Sigma_\Act = \mathscr{P} \left(\bigcup_{s \in S}
\Theta(s)\right) \otimes \Sigma_{\Dist(\St)} \), y llamamos \( \PMeas(\Act) \)
al conjunto de todas las medidas de probabilidad sobre \( \Sigma_\Act \). Cada
conjunto de acciones habilitadas \( \Act(s) \) es medible (es decir, \( \Act(s)
\in \Sigma_\Act \)) y la función \( \theta(s, \cdot, s') \) es medible (es
decir, \( \{a \in \Act \mid \theta(s, a, s') \leq p\} \in \Sigma_\Act \) para
todo \( p \in [0, 1] \)) (ver~\cite{Polytopal}).
...
\begin{definition}[Camino en un PSG]
	...
	Al igual que para juegos estocásticos, denotaremos con $\Paths$ al conjunto de
...
\begin{definition}[Conjuntos soporte y acciones]
	...
	$$\supp((K, \mu)) = \supp(\mu) = \{s \in \St \mid \mu(s) > 0\}.$$
...
\begin{definition}[Estrategia en un PSG]
	Una estrategia $\pi_i$ para el jugador $i$ \\ ($i \in \{\cuad, \diam\}$) en un PSG será una función medible $\pi_i : \Pathsfin^i \rightarrow \PMeas(\Act)$ que asigna una medida de probabilidad a cada $\omega s$ tal que $\pi_i(\omega s)(\Act(s)) = 1$.
\end{definition}

Los distintos tipos de estrategias que presentamos en el
capítulo~\ref{cap:modelos} (puras, aleatorias, sin memoria y con memoria
...
formalmente la definición de $\Prob^{\picuad, \pidiam}_{\Gk,s}$, la medida de
probabilidad definida por la cadena de Markov dada por $\Gk$ y las estrategias
$\picuad$ y $\pidiam$ en el estado $s$. Para eso, para cada $n\geq 0$ y $s \in
	\St$ definimos $\Prob^{\picuad,\pidiam,n}_{\Gk,s}$ sobre rectángulos de caminos
inductivamente como sigue.

Sea $S_i,S_{n-1}\in\powerset(\St)$,
$\polys\in\powerset(\bigcup_{s\in\psgnodes}\psgtrans(s))$, y
$\meass\in\PMeas(\psgnodes)$ para $0\leq i\leq n$ y sea $S_n^\maxplay =
	S_n\cap\psgnmax$ y $S_n^\minplay = S_n\cap\psgnmin$. Definimos entonces:

\vspace{-1cm}
\begin{align*}
	\makebox[.8em]{}
	&\hspace{-.8em}\Prob^{\strat_{\maxplay},\strat_{\minplay},0}_{\StochGK,s}(S_0) =  \Dirac_{s}(S_0), \text{   y} \\[.5ex]
	&\hspace{-.8em}\Prob^{\strat_{\maxplay},\strat_{\minplay},n+1}_{\StochGK,s}(S_0\times(\polys_0\times \meass_0)\times \cdots\times S_n\times(\polys_n\times\meass_n)\times S_{n+1}) = {} \\
	%
	&\int_{(S_0\times(\polys_0\times \meass_0)\times \cdots\times S_n^\maxplay}
	\left(
	\int_{\polys_n\times\meass_n}\sgtrans(\last(\hat{\omega}),(K,\mu), S_{n+1})\ \diff\strat_\maxplay(\hat{\omega})(K,\mu)
	\right)
	\diff\Prob^{\strat_{\maxplay},\strat_{\minplay},n}_{\StochGK,s}(\hat{\omega}) + {} \\
	&\int_{(S_0\times(\polys_0\times \meass_0)\times \cdots\times S_n^\minplay}
	\left(
	\int_{\polys_n\times\meass_n}\sgtrans(\last(\hat{\omega}),(K,\mu), S_{n+1})\ \diff\strat_\minplay(\hat{\omega})(K,\mu)
	\right)
	\diff\Prob^{\strat_{\maxplay},\strat_{\minplay},n}_{\StochGK,s}(\hat{\omega}).
\end{align*}

Sea $\Salg_\Paths$ la $\sigma$-álgebra producto usual sobre los caminos
(infinitos) en el PSG. Por el teorema de extensión de Carathéodory,
$\Prob^{\strat_{\maxplay},\strat_{\minplay}}_{\StochGK,s}:\Salg_\Paths\to[0,1]$
se define como la única medida de probabilidad tal que para todo $n\geq 0$,
$S_i\in\powerset(\psgnodes)$,
$\polys\in\powerset(\bigcup_{s\in\psgnodes}\psgtrans(s))$, y
$\meass\in\PMeas(\psgnodes)$ con $0\leq i\leq n$,%\kathy{Cambio $\GamePaths$.} 
%
\[\Prob^{\strat_{\maxplay},\strat_{\minplay}}_{\StochGK,s}(\Cyl(S_0\times(\polys_0\times \meass_0)\times S_1\times \cdots\times S_n)) = \Prob^{\strat_{\maxplay},\strat_{\minplay},n+1}_{\StochGK,s}(S_0\times(\polys_0\times \meass_0)\times S_1\times \cdots\times S_n)\]
%
donde la funci\'on $\Cyl$ est\'a extendida a conjuntos en la forma usual.
...
acciones. En \cite{Polytopal} se demuestra cómo ciertas preguntas que nos
podemos hacer sobre esta interpretación infinita se pueden responder analizando
una interpretación distinta con una cantidad finita de acciones, la cual
...
La idea es que otra vez estamos en una grilla 2x2, con lo que podemos seguir
representando a los estados de igual manera. Lo que cambiará en este juego, con
respecto al juego estocástico que planteamos para Roborta y Rigoborto en la
sección~\ref{sec:sg}, es que desde los estados en los que les toca elegir a
robots que se encuentran en $(0,0)$ o $(1,1)$ tendremos una cantidad continua
de opciones determinada por un politopo.
...
$p_3$. Estas tres variables respetarán algunas restricciones formando así el
...
\begin{figure}[ht]
	...
	\end{tikzpicture}
	\caption{Fragmento de PSG Roborta vs Rigoborto.}
	\label{fig:psg}
\end{figure}

Resulta también interesante ver algún ejemplo de una estrategia extrema aquí.
...
Luego, un ejemplo de una estrategia extrema sin memoria en nuestro juego podría
...
seguridad (evitar colisiones o estados indeseados) y condiciones de vitalidad
...
PSG que definimos es bastante simple para lo que podría ser. En primer lugar,
solo en los estados en los que les toca elegir a robots que se encuentran en
$(0,0)$ o $(1,1)$ tendremos una cantidad infinita de distribuciones de las
cuales elegir, en el resto la elección de la acción determinará directamente un
comportamiento certero. En segundo lugar, para los estados que tenemos una
cantidad infinita de soluciones, estas solo se corresponden a un politopo
(recordemos que según la definición que dimos, desde un estado podemos tener
una cantidad finita de politopos desde los cuales elegir). Y, en tercer lugar,
los politopos que presentamos no son particularmente complejos. Es valioso
recordar que los politopos pueden tener mayor dimensión y las variables pueden
estar más correlacionadas entre sí. Tomamos la decisión de simplificar lo más
posible el ejemplo a fin de que quede una imagen clara y entendible de qué es
un PSG antes de proceder con los teoremas mostrados en el trabajo de Castro y
D'Argenio \cite{Polytopal}.
...
\cite{Polytopal}. Si bien \cite{Polytopal} aborda 4 tipos distintos de
objetivos (a saber, de alcanzabilidad, de recompensa media, de recompensa total
acumulada y de recompensa total descontada), solo nos concentraremos en los
resultados correspondientes a objetivos de alcanzabilidad.

El primer teorema, establece la determinación de los juegos estocásticos a la
vez que establece que los objetivos de alcanzabilidad en PSGs puede ser
resueltos de manera equivalente en la interpretación extrema del PSG.

\begin{theorem}[Reducción de PSGs]
	Sean $\Gk$ y $\Hk$ la interpretación y la interpretación extrema, respectivamente, de un PSG $\K$. Sea $C$ un subconjunto de estados en $\K$. Entonces vale que:
	\begin{align*}
...
que nos interesa y denotando como $\val_{\Gk,s}(\alc G)$ al valor del estado
$s$ (es decir, la probabilidad máxima de que $\cuad$ gane desde $s$) en el
juego $\Gk$ con un objetivo de alcanzabilidad sobre un conjunto $G$, el teorema
sería:
