...
Para esto, utilizamos una construcción estándar \cite{Giry82} para darle a \(
...
al conjunto de todas las medidas de probabilidad sobre \( \Sigma_\Act \). No es
difícil comprobar que cada conjunto de acciones habilitadas \( \Act(s) \) es
...
$\meass\in\PMeas(\psgnodes)$ para $0\leq i\leq n$ y sea $S_n^\maxplay =
	S_n\cap\psgnmax$ y $S_n^\minplay = S_n\cap\psgnmin$.

\vspace{-1cm}
\begin{align*}
	\makebox[.8em]{}
	&\hspace{-.8em}\Prob^{\strat_{\maxplay},\strat_{\minplay},0}_{\StochGK,s}(S_0) =  \Dirac_{s}(S_0) \\[.5ex]
