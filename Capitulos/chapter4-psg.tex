...
% formalmente la definición de
% $\Prob^{\strat_{\maxplay},\strat_{\minplay}}_{\StochGK,s}$, la medida de
% probabilidad definida por la cadena de Markov dada por $\StochGK$ y las
% estrategias $\strat_{\maxplay}$ y $\strat_{\minplay}$ en el estado $s$. Para
% eso, para cada $n\geq 0$ y $s \in \psgnodes$ definimos
% $\Prob^{\strat_{\maxplay},\strat_{\minplay},n}_{\StochGK,s}$ sobre rectangulos
% de caminos inductivamente como sigue.
% %
% Sea $S_i,S_{n-1}\in\powerset(\psgnodes)$,
...
% Sea $\Salg_\GamePaths$ la $\sigma$-álgebra producto usual sobre $\GamePaths$.
% Por el teorema de extensión de Carathéodory,
% $\Prob^{\strat_{\maxplay},\strat_{\minplay}}_{\StochGK,s}:\Salg_\GamePaths\to[0,1]$
