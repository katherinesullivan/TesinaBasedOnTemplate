...
caminos en $\M$. \kathy{Lo que está a continuación me hace un poco de ruido por
	la parte de la definición de la cadena de Markov. Es como lo presentan en el
	libro de Baier y Katoen. La otra opción es ir poco más con el enfoque de de
	Alfaro y presentar la $\sigma$-álgebra sin mencionar nada de MCs}.
...
MDP $\M$ es finito. \kathy{Entonces, ¿cuenta como una MC? Habría que bajar de
	la def de MC lo de cjto de estado finito en ese caso.}.
...
Podemos pensar en distintas estrategias para controlar a Roborto. Veamos
algunas de ellas y analicemos en qué categorías entrarían:
...
\textbf{Un ejemplo de juego estocástico: Roborto vs Rigoberta}

La idea ahora será presentar un juego estocástico. Para eso pensaremos que
mientras que nosotros tenemos a Roborto, un amigue nuestro tiene un controlador
que le permite decidir los movimientos de Rigoberta, una robota de
características similares a las de Roborto.

Ambos vamos a tener a nuestros robots en el terreno sobre el que antes estaba
Roborto, uno con ciertas inclinaciones que pueden hacer que nuestros comandos
no se ejecuten de manera certera. Y nuestra idea va a ser modelizar esta
...
distintos), además de representar de quién es el turno.
...
\hl{Cambiar esto a algo tipo: observar que siempre son puras acá, pero podemos
	tener lo misma de sin memoria o con memoria finita}
