...
\begin{definition}[Cadena de Markov]
	Una cadena de Markov es una tupla $M = (\St, P)$ donde $\St$ es un conjunto no vacío de estados y $P: \St \times \St \rightarrow [0,1]$ es una función tal que para todos los estados $s$ vale que
...
pensar en los prefijos finitos de ese camino, $\prefijo(\omega) = \{(s_0,
	\dots, s_n) | n \in \mathbb{N}\}$. Sea $M$ una cadena de Markov, denotamos
$\Paths(M)$ al conjunto de todos los caminos en $M$ y $\Pathsfin(M)$ al
...
Un camino en un proceso de decisión de Markov es una secuencia alternante
infinita de estados y acciones, construida iterativamente mediante un proceso
de dos pasos. Primero, dado un estado $s$, una acción $a \in \Act(s)$ es
...
entonces $\G$ es un proceso de decisión de Markov, y, como vimos antes, si,
además, $\forall s \in \St$, $\abs{\Act(s)} = 1$ entonces $\G$ es una cadena de
Markov.
...
\begin{definition}[Estrategia en un juego estocástico]
	Sea $\G = (\St, (\St_\cuad, \St_\diam), \Act, \theta)$ un SG. Una estrategia $\pi_i$ para el jugador $i$ (con $i \in \{\cuad, \diam\}$) en $\G$ es una función $\pi: \Pathsfin^i \rightarrow \dist (\Act)$ que asigna una distribución de probabilidad a cada prefijo finito de camino que termina en un estado del jugador $i$ tal que $\pi(\hat \omega) (a) > 0$ solo si $a \in \Act(s)$.
...
Si considerasemos todas las posiciones en las que se podrían encontrar Roborta
...
adversarial. Para esto, existen muchos modelos matemáticos del concepto de
juego. Para las definiciones que presentaremos a continuación nos basaremos en
lo que se suele conocer en la literatura como juegos de grafo de dos jugadores.
Dentro del estudio de juegos de grafos se presentan varias categorías que se
...
jugador y medio sería lo que se entiende por proceso de decisión de Markov y un
juego de grafo de 2 jugadores y medio sería lo que presentamos como juegos
estocásticos.
...
de estrategia. Esta vez no para poder definir una medida de probabilidad ya que
no necesitaremos una, sino para poder estudiar clases de comportamiento
...
(tanto estocásticos como no-estocásticos) suelen venir equipado con lo que se
