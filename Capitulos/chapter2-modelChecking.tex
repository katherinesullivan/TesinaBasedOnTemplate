...
\epigraph{Compañeros de juego, fiestas y nueces hubo: hubo un imaginar grandioso o delicado. ¡Qué extraño que unos pies con una carga tan preciosa a un umbral tan pequeño hayan llegado!}{\textit{En una noche así}, \\ Emily Dickinson}
...
juegos de dos jugadores tradicionales.
...
Existen numerosas propiedades de interés para comprobar sobre sistemas (a
...
particularidades. La idea de las próximas secciones será presentar varios de
...
\begin{definition}[Cadena de Markov]
	Una cadena de Markov (a tiempo discreto) es una tupla $M = (\St, P)$ donde $\St$ es un conjunto no vacío de estados y $P: \St \times \St \rightarrow [0,1]$ es una función tal que para todos los estados $s$ vale que
...
decir, son definidos como secuencias infinitas de estados $\omega = (s_0, s_1,
	...
	0$. A su vez, dado un camino infinito $\omega = (s_0, s_1, s_2, \dots)$,
podemos pensar en los prefijos finitos de ese camino, $\prefijo(\omega) =
	\{(s_0, \dots, s_n) | n \in \mathbb{N}\}$. Sea $M$ una cadena de Markov,
denotamos $\Paths(M)$ al conjunto de todos los caminos en $M$ y $\Pathsfin(M)$
al conjunto de todos los prefijos finitos de caminos en $M$.
...
conjuntos cilindro (es decir, los eventos) están dadas por:
\begin{equation}
	\label{probMC}
...
las alternativas de ejecución están cuantificadas probabilísticamente, pero
existen situaciones en donde además necesitamos modelar elecciones, es decir,
decisiones no-deterministas. Para ello, existen los llamados procesos de
decisión de Markov.

Un proceso de decisión de Markov es una generalización de una cadena de Markov
donde un conjunto de acciones posibles es asociado a cada estado. A cada par
estado-acción le corresponde una distribución de probabilidad que es usada para
seleccionar el próximo estado. A su vez, una cadena de Markov se corresponde
con un MDP que tiene exactamente una acción asociada a cada estado.
...
\begin{definition}[Proceso de Decisión de Markov]
	Un proceso de decisión de Markov (MDP, por sus siglas en inglés) $\M =(\St, \Act, \theta)$ consiste de un conjunto finito de estados $\St$ y de dos componentes, $\Act$ y $\theta$, que especifican la estructura de transición entre los estados:
	\begin{itemize}
...
Un camino en un MDP es una secuencia alternante infinita de estados y acciones,
construida iterativamente mediante un proceso de dos pasos. Primero, dado un
estado $s$, una acción $a \in \Act(s)$ es seleccionada no-determinísticamente.
Luego, el sucesor $t$ de $s$ es seleccionado de acuerdo a la distribución
asociada a la acción $a$. La definición formal es la siguiente:

\begin{definition}[Camino en un MDP]
	Un camino en un MDP $\M$ es una secuencia infinita $\omega = (s_0, a_0, s_1, a_1, \dots)$ tal que $s \in \St$, $a_i \in \Act(s_i)$ y $\theta(s_i, a_i, s_{i+1}) > 0$ para todo $i \geq 0$.
...
\begin{definition}[Estrategia en un MDP]
	Sea $\M = (\St, \Act, \theta)$ un MDP. Una estrategia para $\M$ es una función $\pi: \Pathsfin \rightarrow \dist (\Act)$ que asigna una distribución de probabilidad a cada prefijo finito de camino, $\hat \omega$, terminado en un estado $s \in \St$. Esta distribución asignada verifica que $\pi(\hat \omega) (a) > 0$ solo si $a \in \Act(s)$.
\end{definition}
...
\begin{definition}[Cadena de Markov de un MDP inducida por una estrategia]
	...
	donde $P_\pi$ es una matriz tal que para todo $\hat \omega = (s_0, a_0, s_1, a_1, \dots, s_n)$, $ a \in \Act(s_n)$ y $s_{n+1} \in \St$,
...
Como $\M_\pi$ es una cadena de Markov, ahora podemos razonar sobre las
...
las estrategias pueden depender de la historia en su totalidad. Sin embargo, es
importante considerar definiciones más restringidas de estrategias que en
muchas ocasiones son suficientes para analizar cierto tipo de propiedades.
Presentaremos algunas de esas estrategias:
...
prefijo finito de camino. Formalmente, la definiremos, siguiendo la definición
propuesta en la tesis doctoral de Luca De Alfaro~\cite{AlfaroThesis}, a través
de un autómata determinista finito (DFA). La distribución de probabilidad de
las acciones será seleccionada a partir del estado actual en $\M$ y el estado
...
$b \neq a$). Y, por otro lado, una estrategia se dice aleatoria cuando no es
...
tales que $\pi(\hat \omega)(s) > 0$ y $\pi(\hat \omega)(s') > 0$). Observar que
en el caso de estar trabajando solo con estrategias puras se puede pensar a las
estrategias como una función $\pi: \Pathsfin \rightarrow \Act$.

Usaremos la notación $\Pi^{M}$ para denotar el conjunto de todas las
estrategias sin memoria, $\Pi^{F}$ para denotar el conjunto de todas las
estrategias de memoria finita, $\Pi^{D}$ para denotar el conjunto de todas las
estrategias puras y $\Pi^{R}$ para denotar el conjunto de todas las estrategias
aleatorias. A su vez, usaremos la notación combinada, por ejemplo, $\Pi^{FD}$
para denotar el conjunto de todas las estrategias de memoria finita y puras.
...
\begin{itemize}
	\item Una estrategia pura y sin memoria podría ser la que siempre decide moverse
	      ...
	      $\leftarrow$.
	\item Una estrategia aleatoria podría ser una en la que desde cada estado, asigne
	      iguales probabilidades de tomar alguna de las dos acciones disponibles. Es
	      decir, por ejemplo tendríamos, $\forall \hat \omega \in \Pathsfin$ tal que
	      $\hat \omega$ termina en $SI$, $\pi(\hat \omega)(\rightarrow) = 0.5$ y
	      $\pi(\hat \omega)(\downarrow) = 0.5$.
	...
	\item Una estrategia con memoria finita podría ser una $\pi$ tal que dado un $\hat
	      ...
	      uno elegirá la acción de moverse verticalmente. Por ejemplo, para el prefijo de
	      camino $\hat \omega' = (SI, II, SI)$ tendremos que $\pi(\hat
...
adversario. En este tipo de comportamiento, surgen distintos agentes con
...
\begin{definition}[Juego estocástico]
	Un juego estocástico es una tupla de cuatro elementos $\G = (\St, (\St_\cuad, \St_\diam), \Act, \theta)$ donde:
	\begin{itemize}
	...
	\end{itemize}
	Denotamos $\Act(s) = \{a \in \Act \mid \theta(s,a,\St) = 1\}$ al conjunto de acciones habilitadas en $s$.
\end{definition}

Se puede notar que si $\St_\cuad = \emptyset$ o $\St_\diam = \emptyset$,
entonces $\G$ es un MDP, y, como vimos antes, si, además, $\forall s \in \St$,
$\abs{\Act(s)} = 1$ entonces $\G$ es una cadena de Markov.

Otra manera de ver a los juegos estocásticos es pensarlos como procesos de
...
% lo que introduce esta idea adversario: no solo habrá un ente que tome las
...
\begin{definition}[Camino en un juego estocástico]
	Un camino en un juego estocástico $\G$ es una secuencia infinita $\omega = (s_0, a_0, s_1, a_1, \dots)$ tal que $\theta(s_i, a_i, s_{i+1}) > 0$ para todo $i \geq 0$.

	También podemos pensar a una acción como una función de probabilidad sobre los
	estados, de manera que la última condición puede ser pensada como $a_i(s_{i+1})
		> 0$. Notación que usaremos más adelante.
...
\begin{definition}[Estrategia en un juego estocástico]
	Sea $\G = (\St, (\St_\cuad, \St_\diam), \Act, \theta)$ un juego estocástico. Una estrategia $\pi_i$ para el jugador $i$ (con $i \in \{\cuad, \diam\}$) en $\G$ es una función $\pi: \Pathsfin^i \rightarrow \dist (\Act)$ que asigna una distribución de probabilidad a cada prefijo finito de camino $\hat \omega$ que termina en un estado $s$ del jugador $i$ tal que $\pi(\hat \omega) (a) > 0$ solo si $a \in \Act(s)$.
...
Podemos ver que las estrategias en un juego estocástico se definen igual a como
...
presentadas en la sección anterior se extienden naturalemente a juegos
estocásticos. Llamaremos $\Pi^{M}_i$ al conjunto de las estrategias sin memoria
del jugador $i$, $\Pi^{F}_i$ al conjunto de las estrategias de memoria finita
del jugador $i$, $\Pi^{D}_i$ al conjunto de las estrategias puras del jugador
$i$, $\Pi^{R}_i$ al conjunto de las estrategias aleatorias del jugador $i$ y
las podremos combinar diciendo por ejemplo que $\Pi^{MD}_i$ es el conjunto de
las estrategias puras sin memoria del jugador $i$ y $\Pi^{FR}_i$ es el conjunto
de las estrategias aleatorias con memoria finita del jugador $i$.
...
denotaremos $\G^{\picuad, \pidiam}$. Esta cadena de Markov, para cada $s \in
	\St$ definirá una medida de probabilidad $\ProbG$ en la $\sigma$-álgebra del
conjunto de caminos en $\G$. Si $\varepsilon$ es un conjunto medible en la
$\sigma$-álgebra, $\ProbG(\varepsilon)$ será la probabilidad de que las
estrategias $\picuad$ y $\pidiam$ sigan un comportamiento en $\varepsilon$
empezando desde el estado $s$.
...
comandos no se ejecuten de manera certera. Nuestra idea va a ser modelar esta
situación a través de un juego estocástico.

Lo que tenemos que tener en cuenta aquí es que, al estar modelando un juego en
donde tenemos dos actores, no podremos simplemente tener como estado algo al
estilo ``esquina superior izquierda'' por el hecho de que nuestro estado tiene
que codificar dónde se encuentran dos robots (que es posible que sean lugares
...
Si considerásemos todas las posiciones en las que se podrían encontrar Roborta
y Rigoborto tendríamos $16$ ($2^4$) estados. Sin embargo, como dijimos, el
estado debería representar de quién es el turno (es decir, quién elije desde
ahí la próxima acción), por eso, en realidad, si pensamos que desde todas las
...
\section{Juegos no-estocásticos}

Ahora bien, dijimos que cuando queremos modelar tanto comportamiento adversario
como probabilístico es cuando necesitamos de juegos estocásticos, pero resulta
que también es muy común querer modelar simplemente comportamiento adversario.
Para esto, existen muchos modelos matemáticos del concepto de juego.

Se suele categorizar a los juegos de acuerdo a la cantidad de jugadores
involucrados, haciendo referencia con ``medio jugador'' a la parte estocástica
de un juego. De esta manera, un juego de grafo de un jugador sería simplemente
un sistema de transición, un juego de grafo de 1 jugador y medio sería lo que
entendemos por MDP y un juego de grafo de 2 jugadores y medio sería lo que
presentamos como juegos estocásticos. Siguiendo esa línea, podemos también
definir un juego de dos jugadores en el que no hay aleatoriedad involucrada. El
juego de grafo de dos jugadores se define formalmente como sigue:

% Para las definiciones que presentaremos a continuación nos basaremos en
% lo que se suele conocer en la literatura como juegos de grafo de dos jugadores.
% Dentro del estudio de juegos de grafo se presentan varias categorías que se
% pueden paralelizar con las definiciones que vimos: un juego de grafo de un
% jugador sería simplemente un sistema de transición, un juego de grafo de 1
% jugador y medio (\textit{one and a half-player graph games}, en inglés) sería
% lo que se entiende por proceso de decisión de Markov y un juego de grafo de 2
% jugadores y medio (\textit{two and a half-player graph games}, en inglés) sería
% lo que presentamos como juegos estocásticos.

% Procedemos entonces con la definición de juego de grafo de dos jugadores:

\begin{definition}[juego de grafo de 2 jugadores]
	Definimos a un juego de grafo de dos jugadores como una tupla $G = (V, V_\cuad,V_\diam, E)$ donde $V = V_\cuad \biguplus V_\diam$ es un conjunto de vértices (o estados) particionado en $V_\cuad$ y $V_\diam$, y $E \subseteq (V\times V)$ es una relación que denota el conjunto de aristas (dirigidas) que representan transiciones de un estado a otro del juego.
...
juegos no-estocásticos volveremos a la noción de camino como secuencia de
vértices solamente, a los cuales notaremos generalmente con $\rho$ en
contraposición a los caminos siendo notados con $\omega$ para juegos
estocásticos.

\begin{definition}[camino sobre un juego de grafo de 2 jugadores]
...
También para el análisis de juegos no-estocásticos introduciremos el concepto
de estrategia. Esta vez no para poder definir una medida de probabilidad, ya
que no necesitaremos una, sino para poder estudiar clases de comportamiento
adversario y poder formular y responder preguntas de investigación que se hacen
en el contexto del estudio de juegos, las que precisaremos con más detalle en
el capítulo siguiente.

% No necesitamos las estrategias para definir una medida de probabilidad pero si
% para analizar el comportamiento adversario -> algo capaz de preguntas acá?

\begin{definition}[estrategia sobre un juego de grafo de 2 jugadores]
...
% \end{definition}
% % algo de aleatorias y deterministas?
...
\begin{figure}[ht]
	...
	\end{tikzpicture}
	\caption{Fragmento de juego no-estocástico: Roborta vs Rigoborto}
...
(tanto estocásticos como no-estocásticos) suelen venir equipados con lo que se
llaman objetivos. A partir de estos objetivos es que podremos pensar en la
pregunta típica: ``¿quién gana?''. Profundizaremos sobre estas nociones en el
próximo capítulo.
